 ...
 * attraversamento. NON possiede nessun sensore/attuatore incorporato:
 * il SensoreQualita (e l'eventuale Deviatore per lo smistamento) sono
 * creati ed agganciati da fuori (dal Controllore, tramite
 * controllore_collegaSensoreQualita), con lo stesso schema usato per
 * SensoreBuffer/SensorePresenza sui buffer e per Motore/Deviatore su
 * nastri/ISP: isp_tryRelease restituisce solo l'oggetto controllato
 * (come machine_tryRelease), il calcolo dell'esito qualita'
 * (get_qualita) e la scelta del materiale (get_Material) avvengono nel
 * Controllore usando il sensore agganciato. Se nessun sensore e' stato
 * agganciato a una ISP (es. una ISP "passacarte" con una sola uscita),
 * quella ISP semplicemente non fa nessun controllo qualita', senza
 * bisogno di nessun target fittizio.
 */
 ...
 *        la ISP di per se' non sa nulla di qualita', e' un puro timer
 *        come machine_t. Il calcolo dell'esito (se serve) spetta al
 *        Controllore, che ha il sensore agganciato tramite
 *        controllore_collegaSensoreQualita.
 * @param i Puntatore alla ISP.
 * @param step_corrente Step di simulazione corrente.
