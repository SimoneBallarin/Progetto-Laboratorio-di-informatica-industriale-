...
{
    if ( cell == NULL || scenario == NULL ) return ERR_NULL_PTR;
    if ( scenario->guasto_isp_id[0] == '\0' ) return OP_SUCCESS; /* nessun guasto da configurare */

    isp_t *isp = cell_getISP( cell, scenario->guasto_isp_id );
    if ( isp == NULL ) {
        fprintf( stderr, "[parser] scenario: ISP '%s' non trovata nella cella\n", scenario->guasto_isp_id );
